...
"""
function GaussMarkovRandomField(ρ::Number, img::AbstractMatrix; order::Integer = 1, executor = Serial())
    cache = MarkovRandomFieldGraph(eltype(img), size(img); order, executor)
...
"""
function GaussMarkovRandomField(ρ::Number, dims::Dims{2}; order::Integer = 1, executor = Serial())
    cache = MarkovRandomFieldGraph(typeof(ρ), dims; order, executor)
    return GaussMarkovRandomField(ρ, cache)
end
